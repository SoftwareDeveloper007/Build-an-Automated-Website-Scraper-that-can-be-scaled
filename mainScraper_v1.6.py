...
class mainScraper():
    def __init__(self, infile, inpath):
        ...
        #self.output = open('Result/' + self.infile, 'w', encoding='utf-8', newline='')
    ...
    def inputFile(self):
        try:
            ...
            if self.infile.endswith('.csv'):
                input_csv = open(self.inpath + '\\'+ self.infile, 'r')
                reader = csv.reader(input_csv)

                for i, row in enumerate(reader):
                    if i == 0:
                        continue
                    self.input_data.append([i] + row + ['']*(7-len(row)))
                self.input_data.reverse()
                logTxt = 'Input csv file successfully'
                print(logTxt)
            else:
                input_xls = xlrd.open_workbook(self.inpath + '\\'+ self.infile)
                sheet = input_xls.sheet_by_index(0)
                for i in range(sheet.nrows):
                    if i == 0:
                        continue
                    self.input_data.append([i] + [sheet.row(i)[j].value for j in range(len(sheet.row(i)))])
                self.input_data.reverse()
                logTxt = 'Input csv file successfully'
                print(logTxt)
    ...
    def extractDetails(self):
        ...
        if row[2] is '':
            ...
            try:
                ...
                try:
                    yelp_friends = member_row.find_element_by_css_selector(
                        "li.friend-count.responsive-small-display-inline-block").text
                except:
                    yelp_friends = ''

                try:
                    yelp_reviews = member_row.find_element_by_css_selector(
                        "li.review-count.responsive-small-display-inline-block").text
                except:
                    yelp_reviews = ''
    ...
    def saveCSV(self):
        ...
        output = open('Result/' + self.infile, 'w', encoding='utf-8', newline='')
...
if __name__ == '__main__':
    ...
    # app.startScraping()
    # app.saveCSV()
    for (path, dirs, files) in os.walk(os.getcwd() + "\\Partially Complete"):
